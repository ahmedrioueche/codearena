        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {languages.map((lang) => (
            <button
              key={lang.id}
              onClick={() =>
                setMatchConfig({ ...matchConfig, language: lang.id })
              }
              className={`p-4 rounded-xl border-2 transition-all duration-200 hover:scale-105 ${
                matchConfig.language === lang.id
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {topics.map((topic) => (
            <button
              key={topic.id}
              onClick={() => {
                const newTopics = matchConfig.topics.includes(topic.id)
                  ? matchConfig.topics.filter((t) => t !== topic.id)
                  : [...matchConfig.topics, topic.id];
                setMatchConfig({ ...matchConfig, topics: newTopics });
              }}
              className={`p-4 rounded-xl border-2 transition-all duration-200 hover:scale-105 ${
                matchConfig.topics.includes(topic.id)
                  ? "border-light-primary dark:border-dark-primary bg-light-primary/5 dark:bg-dark-primary/10"
                  : "border-light-border dark:border-dark-border"
              }`}
            >
